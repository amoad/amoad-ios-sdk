//
//  NativeHtmlSwiftViewController.swift
...
class NativeHtmlSwiftViewController: UIViewController, UIWebViewDelegate {
  ...
  override func viewDidLoad() {
    ...
    if NativeHtmlSwiftViewController.LOAD_FROM_URL {
      // URLからロードする
      if let requestURL = URL(string: "https://example.com/") {  // TODO: URLを指定する
        let req = URLRequest(url: requestURL)
        self.webView.loadRequest(req)
      }
    } else if NativeHtmlSwiftViewController.LOAD_FROM_FILE {
      // ファイルからロードする
    ...
    } else if NativeHtmlSwiftViewController.LOAD_FROM_STRING {
      // 文字列からロードする
